...
$query="insert into  registration(roomno,seater,personproom,roomtype,feespm,landlordphone,landlordname,foodstatus,stayfrom,duration,course,regno,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,guardianName,guardianRelation,guardianContactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiissisisisissssisississsisssi',$roomno,$seater,$personproom,$roomtype,$feespm,$landlordphone,$landlordname,$foodstatus,$stayfrom,$duration,$course,$regno,$fname,$mname,$lname,$gender,$contactno,$emailid,$emcntno,$gurname,$gurrelation,$gurcntno,$caddress,$ccity,$cstate,$cpincode,$paddress,$pcity,$pstate,$ppincode);
$stmt->execute();
...
$uid=$_SESSION['login'];
				...
				if($rs)
				{ ?>
			<h3 style="color: green" align="left">Hostel already booked by you</h3>
				<?php }
				else{
							echo "";
							}			
